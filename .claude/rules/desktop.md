...
- **The menu bar draws `icons/tray.png`, not the app icon.** macOS templates use the alpha
  channel alone, and the opaque app icon templates into a solid blob, so the tray has its
  own transparent monochrome glyph (regenerated by `tools/tray_icon.py`), embedded with
  `include_bytes!` so that cargo rebuilds when it changes - icons that go through
  `generate_context!` are not tracked and can survive a rebuild stale.
- **The tray counts nothing itself:** every five seconds it takes `/api/overview`. The
  polling lives in its own thread: the menu bar lives without the window. There is no alert
  mark in the menu bar - the title is drawn monochrome, so the once-planned red dot could
  never be red and was dropped; "pause for 2 hours" holds the telegram messages.
