...
- **Сессию с процессом связывает только рабочий каталог.** `sessionId` не
  попадает ни в аргументы процесса, ни в открытые дескрипторы (транскрипт
  дописывается и закрывается). Завершать процесс можно, лишь когда каталогу
  отвечает ровно один `claude`, иначе дашборд отказывается гадать.
