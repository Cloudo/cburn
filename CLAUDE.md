...
- **Где эти данные применяются:** виджет «мимо транскриптов» на дашборде
  (служебный расход, ручные подтверждения, сорвавшиеся запросы к API), время
  в инструментах на экране сессии, секции `off_transcript` и `permissions`
  в дайджесте советчика и определение статуса «ждёт разрешения».
