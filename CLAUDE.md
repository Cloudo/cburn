...
React + Vite (появится на M2, каталог `web/`). Пакетный менеджер — pip в `.venv`,
`uv` в системе нет.
...
- **`user`-записи двух сортов**: настоящий промпт (есть `promptSource`) и
  `tool_result` — для «первого промпта» и эвристик это разные вещи.
- **Сабагенты** помечены `isSidechain` и лежат в файле родительской сессии.
- Ходы между файлами не дублируются; `turns.uuid` — страховка от повторного
  импорта хвоста, а не механизм склейки форков.
