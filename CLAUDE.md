...
- **Запись ≠ ход**: один ответ ассистента разложен по нескольким JSONL-записям
  (по одной на блок `thinking`/`text`/`tool_use`), и в каждой лежит полный и
  одинаковый `usage`. Ключ хода — `message.id`; суммирование по записям завышает
  расход примерно вчетверо.
