...
- **Личные атрибуты не сохраняются.** В каждой точке едут `user.email`,
  `user.account_id`, `organization.id` и прочее одинаковое для машины —
  в БД они не нужны и отфильтровываются (`SKIPPED_ATTRS` в `collector/otlp.py`).
