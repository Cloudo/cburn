...
`permissions` — сколько раз работа вставала ради ручного подтверждения и по
каким инструментам (повод предложить правку `permissions.allow`), и
`off_transcript` — служебные запросы, которых в транскриптах нет вовсе, то
есть остальные цифры дайджеста на эту величину занижены. Если у секции
`available: false`, телеметрия выключена — молчи про них, а не считай нулём.
