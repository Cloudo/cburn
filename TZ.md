# ТЗ: «Спидометр Claude» (рабочее имя: claude-speedo)
...
localhost:4317 (OTLP, M4) ─────────────▶│
                                        ▼
                                   SQLite (~/.local/share/claude-speedo/speedo.db)
                                        │
              ┌─────────────────────────┼──────────────────────┐
              ▼                         ▼                      ▼
   ...
   Запуск: `speedo serve` (один процесс, uvicorn) или launchd-агент
...
`~/.config/claude-speedo/config.toml`, редактируется и из UI:
...
БД восстанавливается полной переиндексацией командой `speedo reindex`.
...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| M1   | CLI-прототип: парсер JSONL, SQLite, команды `speedo stats`, `speedo sessions`, `speedo session <id>`                                    | Цифры по одной реальной сессии совпадают с ручным подсчётом usage-полей; полная индексация истории машины проходит без падений                        |
| M2   | API-сервер + веб-дашборд: экраны Обзор, Сессии, Сессия, Настройки; live через WebSocket; `speedo serve` + launchd-агент для автозапуска | Открытый в браузере дашборд показывает ход текущей сессии Claude Code с задержкой ≤ 1 с                                                               |
