# ТЗ: «Спидометр Claude» (рабочее имя: cloudo-dash)
...
localhost:4317 (OTLP, M4) ─────────────▶│
                                        ▼
                                   SQLite (~/.local/share/cloudo-dash/cloudo-dash.db)
                                        │
              ┌─────────────────────────┼──────────────────────┐
              ▼                         ▼                      ▼
   ...
   Запуск: `cdash serve` (один процесс, uvicorn) или launchd-агент
...
`~/.config/cloudo-dash/config.toml`, редактируется и из UI:
...
БД восстанавливается полной переиндексацией командой `cdash reindex`.
...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| M1   | CLI-прототип: парсер JSONL, SQLite, команды `cdash stats`, `cdash sessions`, `cdash session <id>`                                    | Цифры по одной реальной сессии совпадают с ручным подсчётом usage-полей; полная индексация истории машины проходит без падений                        |
| M2   | API-сервер + веб-дашборд: экраны Обзор, Сессии, Сессия, Настройки; live через WebSocket; `cdash serve` + launchd-агент для автозапуска | Открытый в браузере дашборд показывает ход текущей сессии Claude Code с задержкой ≤ 1 с                                                               |
