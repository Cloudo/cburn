...
- [ ] **A2. Импорт в БД.** `collector/indexer.py`: `ingest_file(conn, path)` — чтение
      ...
      Правка схемы под находки A1 (БД пока пересоздаётся, миграций нет):
      `turns.uuid UNIQUE` → `turns.message_id UNIQUE`, ход собирается из нескольких
      записей (usage от первой, `tool_calls` — со всех записей хода); при resume ход
      уже есть в БД под другим `session_id` — приписывать его первой увидевшей сессии
      и не задваивать расход.
