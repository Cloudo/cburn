...
- [x] **B3. Метрики ТЗ §4.** SQL-слой `src/cloudo_dash/metrics.py`: burn rate в окнах
      ...
      «на что уходят ходы» на дашборде. Окно лимитов восстанавливается по данным —
      Claude Code не пишет в транскрипт ни границ окна, ни самих лимитов, поэтому
      отдаётся объём, а не проценты, и помечено приближением.
