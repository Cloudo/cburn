...
на 14 августа 2026 — сделано 34 задачи из 36, вехи A, B, C, E и F закрыты целиком;
остались только уведомления в telegram (D4, D5), отложенные по решению.
...
- [x] **F3.** Сборка одного `.app`, автозапуск вместо launchd.
  ...
  тот молчит: команда ищется по `CLOUDO_DASH_SERVE`, затем в привычных местах
  ...
  autostart), так что `cdash install` для десктопного сценария больше не
