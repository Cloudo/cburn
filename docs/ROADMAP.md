...
Сводка на 13 августа 2026: сделано 12 задач из 27. Ниже — зачем нужна каждая
оставшаяся, чтобы решать «что брать следующим» по отдаче, а не по порядку.

| Задача | Что даёт |
| ------ | -------- |
| **B4. Живость** | Закончившаяся сессия перестаёт висеть в «сейчас в работе» целый час. Сверка с `claude agents --json`: на реальном прогоне из 28 «простаивающих» живыми были 4, остальные — хвост пакетной работы |
| **B5. Форки и сабагенты** | `parent_session_id` и цепочки resume: сейчас одна линия работы, продолженная после `/clear`, выглядит как несколько несвязанных сессий. Плюс расход сабагентов отдельной пометкой |
| **B6. `raw_events`** | Когда Claude Code сменит формат, будет видно, *что* именно изменилось, а не просто просадка цифр |
| **B7. CLI целиком** | `cdash stats` с фильтрами по проекту и периоду: разбор без браузера и удобная проверка цифр |
| **B8. Смоук на всей истории** | Страховка от тихой поломки парсера после обновления Claude Code |
| **C1. Экран «Сессии»** | История с фильтрами: сейчас видно только топ-5 и живые |
| **C2. Экран «Сессия»** | График контекста по ходам: видно, *где* сессия раздулась и когда пора `/clear` |
| **C3. Экран «Настройки»** | Правка конфига (цены, пороги, telegram) из интерфейса |
| **C4. Self-cost советчика** | Честная стоимость самого советчика на «Обзоре». Прогресс индексации после B2 не нужен |
| **C5. Автозапуск** | launchd: дашборд поднимается сам и переживает перезагрузку |
| **D1. Дайджест** | Агрегаты периода без LLM. Здесь же держится приватность: наружу идут только имена инструментов, нормализованные команды и числа |
| **D2. `claude -p`** | Собственно советы: что вынести в скилл, где поправить permissions, какой MCP отключить |
| **D3. Планировщик** | Такт раз в час, пропуск пустых тактов |
| **D4. `/notify` в бридже** | Отдельная задача в соседнем `~/code/cc-tg-bridge` |
| **D5. Notifier** | Три типа сообщений в telegram: часовой дайджест, тревога по burn rate, дневная сводка |
| **D6. Экран «Советы»** | История советов со статусами; отклонённые не повторяются |
| **D7. Приёмка M3** | На реплее истории советчик находит то, что было найдено руками по navuik/core |
| **E1–E3. OTel** | Точные лимиты подписки вместо приближения и гранулярность тоньше 5 секунд |
| **F1–F3. Десктоп** | Tauri без правок фронта, стрелка в меню-баре с красной зоной, один `.app` |

**Веха D — ради чего всё затевалось.** Всё до неё строит приборы; советчик
единственный сам говорит, что поменять.

Развилка после B4: добирать B5–B8 (качество данных и удобство) или прыгать на D1.
Советчику хватает метрик, закрытых в B3, но без B5 дайджест не увидит связь
resume-форков — линия работы, продолженная после `/clear`, размажется по трём
сессиям вместо одной. Ровно тот случай из ручного отчёта по navuik/core.
