...
const DICT: Record<string, [string, string]> = {
  ...
  "advice.state": ["что с советом", "what became of it"],
  "advice.state.new": ["не разобрано", "to decide"],
  "advice.state.accepted": ["принято", "accepted"],
  "advice.state.rejected": ["отклонено", "rejected"],
  "advice.state.all": ["все", "all"],
  ...
  "advice.nothingShown": [
    "под выбранное ничего не подходит - расширьте окно или посмотрите другой статус",
    "nothing matches the current cuts - widen the window or look at another state",
  ],
