...
Три повода написать, и все про «посмотри, тут не так»: часовая выжимка, если
советчик нашёл что-то важнее `info`; дневная сводка после назначенного часа
(один раз за день); мгновенная тревога, когда расход выше порога или контекст
сессии перевалил за критический.

Настраивается в секции `[telegram]`: `mode` — `bridge` (по умолчанию), `bot`
или `off`; `bridge_url` — адрес соседнего `cc-tg-bridge`, у которого для этого
есть `POST /notify`; `bot_token` и `chat_id` — для прямого Bot API;
`daily_summary_at` — время сводки по местным часам.

Одна сессия не будит чаще раза в полчаса. Пункт «пауза на 2 часа» в меню-баре
(или `POST /api/notify/pause`) держит тишину, но `crit` сквозь неё проходит:
пауза значит «не отвлекай по мелочам», а не «выключи прибор».
