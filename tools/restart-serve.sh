#!/usr/bin/env bash
# Перезапуск дашборда: остановить работающий `cburn serve` и поднять заново
...
# Логи:   ~/.local/share/cburn/serve.log
...
[ -x "$CBURN" ] || { echo "нет $CBURN — соберите окружение: pip install -e '.[dev]'" >&2; exit 1; }

# Порт берётся из конфига, а не зашит: его правят в config.toml.
PORT="$("$CBURN" paths | awk '/^порт/ {print $NF}')"
PORT="${PORT:-8799}"
