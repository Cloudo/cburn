...
# and refresh the resources with:  brew update-python-resources cburn
class Cburn < Formula
  ...
  def caveats
    <<~EOS
      ...
      To have it come up at login:
        cburn install

      The frontend is packed into the archive, so nothing has to be built by hand.
    EOS
  end

  test do
    assert_match "cburn", shell_output("#{bin}/cburn --help")
    # `paths` touches nothing and prints where the database and the config would go.
    assert_match "database", shell_output("#{bin}/cburn paths")
  end
end
